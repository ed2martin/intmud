...
LONG1 h0, h1, h2, h3, h4;
LONG1 A, B, C, D, E;

local void byteReverse (LONG1 *buffer, int byteCount);
...
{
	/* Update bitcount */
	if ((shsInfo->countLo + ((LONG1) count << 3)) < shsInfo->countLo)
		 shsInfo->countHi++;	/* Carry from low to high bitCount */
	shsInfo->countLo += ((LONG1) count << 3);
	shsInfo->countHi += ((LONG1) count >> 29);
...
{
	int count;
	LONG1 lowBitcount = shsInfo->countLo, highBitcount = shsInfo->countHi;
	 ...
	 */
	((BYTE1 *) shsInfo->data) [count++] = 0x80;

	/* Pad out to 56 mod 64 */
	if (count > 56) {
		/* Two lots of padding:  Pad the first block to 64 bytes */
		memset ((BYTE1 *) shsInfo->data + count, 0, 64 - count);
		byteReverse (shsInfo->data, SHS_BLOCKSIZE);
		shsTransform (shsInfo);
		...
		memset ((BYTE1 *) shsInfo->data + count, 0, 56 - count);
	byteReverse (shsInfo->data, SHS_BLOCKSIZE);
