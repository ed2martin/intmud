...
#define TESPERA_MAX 30  // Tempo m�ximo que pode esperar (10 = 1 segundo)
    ...
    for (; *mens; mens++)
    {
        ...
        case 'd':
            ...
            if (tamanho < 0)
            {
                if (destino < fim)
                    *destino++ = '-';
                // Cuidar do overflow quando tamanho == INT_MIN
                if (tamanho == INT_MIN)
                    tamanho = INT_MAX; // Melhor aproximação possível
                else
                    tamanho = -tamanho;
            }
...
// Envia mensagem
    const char msg1[] = "IntMUD vers�o " VERSION " (Interpretador MUD)\n";
    if (!opcao_log)
    {
        if (Console == nullptr)
        {
...
{
#ifdef __WIN32__
    // Nota: a base de tempo do IntMUD � de 100 milissegundos
    // Se for necess�rio maior precis�o, usar QueryPerformanceFrequency()
...
// Processamento de eventos
    ...
    // Obt�m: espera = tempo decorrido
        ...
        atual -= espera % 100;  // Para passar para d�cimos de segundo
        espera /= 100;          // Arredonda para d�cimos de segundo
        if (espera > 0x800)
            espera = 0;
        if (espera > TESPERA_MAX)
            espera = TESPERA_MAX;

    // Apaga vari�veis de textovar
...
//------------------------------------------------------------------------------
/// Adiciona arquivos das instru��es incluir
...
/// @param arg Nome do arquivo .int, ou "" se n�o foi especificado
...
// Obt�m nome do programa: arqnome e arqinicio
    {
        char nome[0xC000];
        char * pnome = nome;
    ...
    // Obt�m o nome do arquivo principal, sem o diret�rio
        while (true)
        {
    ...
    // Coloca em arqnome o nome do diret�rio atual seguido de uma barra
#ifdef __WIN32__
...
// Cria classes a partir dos arquivos .int
    ...
    // L� pr�xima linha
        ...
        {
        // Fim da configura��o: adiciona arquivos a serem buscados
            if (ini_arq)
            {
                ini_arq = false;
                AdicionaIncluir();
            }
        // Pasa para o pr�ximo arquivo
            mapa = mapa->Proximo;
            if (mapa == 0)
                break;
    ...
    // Configura��o
        ...
        // Verifica instru��es incluir
            if (comparaZ(mens, "incluir") == 0)
            {
                if (!TArqMapa::NomeValido(valor, true))
                {
                    err_printf("Nome n�o permitido ao incluir: %s\n", valor);
                    err_fim();
                }
    ...
    // Verifica se � defini��o de classe
    ...
    // Verifica se classe � v�lida ou j� existe
        ...
        {
            err_printf("%s:%d: Classe inv�lida: %s\n",
...
// Limpa a informa��o de onde est� cada classe
...
// Acerta TClasse::Comandos
    ...
    // Verifica classe
        if (pclasse)
        {
            ...
            {
                err_printf("%s:%d: Classe n�o encontrada: %s\n",
                            arqinicio, linhanum, pclasse);
                erro = true;
                break;
            }
    ...
    // Instru��es antes da defini��o da classe
        if (classeatual == nullptr)
        {
            err_printf("%s:%d: Instru��es n�o pertencem a nenhuma classe\n",
                        arqinicio, linhanum);
            erro = true;
            continue;
        }
    ...
    // Verifica instru��o
        if (comando[2] == Instr::cHerda)
        {
            ...
            for (unsigned char x = comando[3]; x; x--)
            {
                ...
                if (obj != classeatual)
                    continue;
                err_printf("%s:%d: Imposs�vel herdar a pr�pria classe\n",
                            arqinicio, linhanum);
                erro = true;
                continue;
            }
