...
#include <stdlib.h>
 ...
 #include <fcntl.h>
 #include <netdb.h>
 #include <netinet/in.h> // Cont�m a estrutura sockaddr_storage
 #include <string.h>
 #include <sys/time.h>
 #include <sys/types.h>
...
// Ocorreu algum erro
    // UNIX: o erro est� em strerror(errno)
...
// Recebe conex�es
    ...
    {
    // Verifica se tem conex�es pendentes
    ...
    // Recebe conex�es
        ...
        while (obj == varObj)
        {
            ...
            if (obj->modossl)
                new TVarServObj(obj, localSocket); // Conex�o segura
            else
...
// Negocia conex�es SSL
    for (TVarServ * obj = Inicio; obj; )
    {
        ...
        // Checa tempo tentando conectar
            ...
            x->tempo -= tempo;
        // Prossegue com a conex�o
            int resposta = SslAccept(x->sockssl);
            if (resposta > 0) // Conex�o realizada
            {
            // Apaga objeto x sem fechar a conex�o
...
bool TVarServ::FuncIniSSL(TVariavel * v)
{
    ...
    if (!arqvalido(arq_crt, true))
        return "Nome do arquivo CRT n�o � permitido";
    if (!arqvalido(arq_key, true))
        return "Nome do arquivo KEY n�o � permitido";
