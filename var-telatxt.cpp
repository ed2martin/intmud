...
// Envia texto
    ...
    for (; tamanho > 0; texto++, tamanho--)
    {
        ...
        // Atualiza buffer com conte�do da tela
...
// Tecla normal (texto)
    ...
    // Est� no fim da linha na tela
        {
            col_linha++;
            Console->CursorIni(); // Cursor no in�cio da linha
...
// ENTER
    if (strcmp(texto1, "ENTER") == 0)
    {
        ...
        tam_linha = 0;
        ColEditor = 0;
        CursorEditor();        // Cursor na linha de edi��o
        Console->CursorIni();  // Posicina o cursor no in�cio da linha
...
// DEL
    ...
    // Resultado na tela
        ...
        Console->LimpaFim(); // Limpa da posi��o do cursor at� o final
    ...
    case TelaTxtTotal:
        {
            ...
            snprintf(txtnum, 0x400, "%u", max_linha);
            return txtnum;
        }
    case TelaTxtLinha:
        {
            char * txtnum = TVarInfo::BufferTxt();
            snprintf(txtnum, 0x400, "%d", LinhaPosic);
            return txtnum;
        }
    }
    return "";
...
void TVarTelaTxt::addTxt(int numfunc, const char * txt)
{
    ...
    col_linha = 0xFFF;
    CursorEditor();     // Cursor na linha de edi��o
    ProcTeclaCursor(0); // Semelhante � tecla HOME
}
...
// Acerta buffer com c�pia da tela
    if (TelaBuf)
        delete[] TelaBuf;
...
void TVarTelaTxt::Processa()
{
    ...
    // L� tecla
        ...
        if (p == nullptr)
            break;
        //Console->Escrever(p);
    // Altera posi��o Y se for evento do mouse
...
bool TVarTelaTxt::FuncEvento(const char * evento, const char * texto)
{
    ...
    // Definido em objeto: prepara para executar
        ...
        {
            char mens[80];
            snprintf(mens, sizeof(mens), "%s_%s", vobj->defvar+Instr::endNome, evento);
...
//------------------------------------------------------------------------------
inline void TVarTelaTxt::FOperadorAtribSub(int numfunc, TVariavel * v2)
{
    if (Console == nullptr)
        return;
    if (numfunc == TelaTxtTexto)
    {
    ...
    else if (numfunc == TelaTxtTotal)
    {
        ...
        else
        {
            ...
            if (col_linha + ColEditor > (unsigned int)valor)
                ProcTeclaCursor(tam_linha); // Semelhante � tecla END
...
//------------------------------------------------------------------------------
void TVarTelaTxt::FOperadorAtrib(TVariavel * v1, TVariavel * v2)
{
