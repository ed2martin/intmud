...
// Acerta tabTXTSEPARA
    ...
    tabTXTSEPARA[(unsigned char)' '] = 8; // Espa�o
    for (int x = '0'; x <= '9'; x++) // D�gitos
        tabTXTSEPARA[x] = 4;
...
#ifndef __WIN32__
    struct stat buf;
    if (stat(nome, &buf) < 0) // Obt�m dados do arquivo
        return true;
    if (!S_ISDIR(buf.st_mode) && // N�o � diret�rio
         (buf.st_mode & (S_IXUSR | S_IXGRP | S_IXOTH))) // � execut�vel
        return false;
#endif
    return true;
}
...
// Copia texto conforme vari�vel opcoes
...
/// L� texto do clipboard
/** @return O texto ou 0 se nao conseguiu ler
 *  @note � responsabilidade de quem chama desalocar a mem�ria com delete[] */
char * ClipboardLer()
{
#ifdef __WIN32__
    const int codePage = 1252; // 1252=windows-1252, CP_UTF8=UTF-8
...
void TAddBuffer::Add(const char * origem, int tamanho)
{
    ...
    while (tamanho > 0)
    {
        ...
        if (copiar <= 0) // Se o bloco est� cheio
        {
